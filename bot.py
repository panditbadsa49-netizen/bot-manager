...
async def handle_group_keywords(update, context, msg, user):
    ...
    if update.effective_chat.type != 'private':
        ...
        # ১. অ্যাডমিনদের আইডি চেক করা (অ্যাডমিনরা লিখলে বট রিপ্লাই দেবে না)
        try:
            ...
            # যদি অ্যাডমিন লিস্ট পেতে সমস্যা হয় (বট অ্যাডমিন না হলে), তবে সাধারণ মেম্বার হিসেবে কাজ করবে
            pass

        # ২. বিশাল কি-ওয়ার্ড লিস্ট (রিসার্চ করে বাড়ানো হয়েছে)
        keywords = [
            # বেসিক গ্রিটিংস
            "IT", "HELLO", "HI", "HLO", "HEY", "হ্যালো", "হাই", "আসসালামু আলাইকুম", "সালাম", "SLA", "SLM",
            # নতুন মেম্বার সংক্রান্ত
            "আমি নতুন", "নতুন মেম্বার", "NOTUN MEMBER", "AMI NOTUN", "I AM NEW", "NEW MEMBER", "নতুন জয়েন করলাম",
            # কাজ সংক্রান্ত প্রশ্ন
            "কি কাজ", "কাজ কি", "কিভাবে কাজ করে", "কাজটা কি", "কি কাজ করতে হবে", "KI KAJ", "KAJ KI", "KAJ TA KI",
            "কাজ করতে চাই", "কাজ দিন", "KAJ KORTE CHAI", "চাকরি", "JOB", "কাজ আছে", "কাজ কি আছে",
            "কি করতে হবে", "KI KORTE HOBE", "কিভাবে শুরু করব", "HOW TO START", "কাজের প্রসেস কি",
            # পেমেন্ট ও ইনকাম
            "পেমেন্ট", "টাকা", "বেতন", "SALARY", "PAYMENT", "টাকা পাবো কিভাবে", "বিকাশ", "BKASH", "INCOME", "ইনকাম",
            # হেল্প ও ডিটেইলস
            "হেল্প", "HELP", "সাহায্য", "বিস্তারিত", "DETAILS", "DETAILS PLZ", "একটু বুঝিয়ে বলুন", "বুঝিয়ে দাও",
            # শর্টকাট ও বানান ভুল (Fuzzy logic এ আরও ধরা পড়বে)
            "KJ KI", "KAJ", "PLZ HELP", "কিভাবে কি", "কাজ বুঝায় দেন"
        ]
        
        # মেসেজ প্রসেসিং
        incoming_msg = msg.strip().upper() if msg else ""
        words_in_msg = incoming_msg.split()
        found_match = False
        
        # ৩. বানান ভুলের জন্য ৫% থেকে ৬০% মিল চেক (Fuzzy Match)
        for word in words_in_msg:
            # কি-ওয়ার্ডের সাথে শব্দের মিল চেক
            matches = difflib.get_close_matches(word, [k.upper() for k in keywords], n=1, cutoff=0.6)
            if matches:
                found_match = True
                break
        
        # যদি হুবহু কোনো কি-ওয়ার্ড বাক্যের ভেতর থাকে (যেমন: "ভাই কাজ কি আছে?")
        if not found_match:
            for key in keywords:
                if key.upper() in incoming_msg:
                    found_match = True
                    break

        # ৪. ম্যাচ করলে রিপ্লাই দেওয়া
        if found_match:
            # আপনার টেলিগ্রাম লিংক (এখানে আপনার লিংক দিন)
            my_link = "https://t.me/skyzoneit/6300" 
            
            reply_text = (
                f"👋 আসসালামু আলাইকুম {user.mention_html()}!\n\n"
                f"আমাদের গ্রুপে আপনাকে স্বাগতম। আপনি যেহেতু কাজের বিষয়ে জানতে চাচ্ছেন, "
                f"তাই নিচে দেওয়া লিঙ্কে ক্লিক করে আমাদের কাজের নিয়মাবলী এবং বিস্তারিত তথ্য পড়ে নিন:\n\n"
                f"👉 <b><a href='{my_link}'>এখানে ক্লিক করে বিস্তারিত পড়ুন</a></b>\n\n"
                f"সবকিছু মনোযোগ দিয়ে পড়ার পর, সরাসরি কাজের জন্য আমার ইনবক্সে গিয়ে <b>'IT'</b> লিখে একটি মেসেজ দিন। "
                f"আমি আপনাকে কাজ বুঝিয়ে দেব। ধন্যবাদ!"
            )
            
            await update.message.reply_text(
                reply_text, 
                parse_mode=ParseMode.HTML,
                disable_web_page_preview=False # লিঙ্ক প্রিভিউ দেখাবে
            )
            return
